...
import { formatDate, extractDomains, validateRules } from "../lib/utils.js";
...
export async function runTests(): Promise<void> {
  ...
  runner.test("Date formatting", () => {
    ...
    if (ddmmyyyy !== "28052025") {
      throw new Error(
        `DDMMYYYY format failed: expected "28052025", got "${ddmmyyyy}"`,
      );
    }
    if (ddmmyyyy_dash !== "28-05-2025") {
      throw new Error(
        `DD-MM-YYYY format failed: expected "28-05-2025", got "${ddmmyyyy_dash}"`,
      );
    }
  });
  ...
  runner.test("Domain extraction", () => {
    ...
    for (const expected of expectedDomains) {
      if (!domains.includes(expected)) {
        throw new Error(
          `Expected domain ${expected} not found in: ${domains.join(", ")}`,
        );
      }
    }
    ...
    if (domains.length !== expectedDomains.length) {
      throw new Error(
        `Expected ${expectedDomains.length} domains, got ${domains.length}`,
      );
    }
  });
  ...
  runner.test("Rule validation", () => {
    ...
    const hasEmptyLines = validationResult.validRules.some(
      (rule) => rule.trim() === "",
    );
    if (hasEmptyLines) {
    ...
    const expectedNonEmptyRules = testRules.filter(
      (rule) => rule.trim() !== "",
    ).length;
    if (validationResult.validRules.length !== expectedNonEmptyRules) {
      throw new Error(
        `Expected ${expectedNonEmptyRules} valid rules, got ${validationResult.validRules.length}`,
      );
    }
  });
  ...
  runner.test("AdblockGeneratorError", () => {
    ...
    if (error.name !== "AdblockGeneratorError") {
      throw new Error(
        `Expected error name to be "AdblockGeneratorError", got "${error.name}"`,
      );
    }

    if (error.code !== "TEST_ERROR_CODE") {
      throw new Error(
        `Expected error code to be "TEST_ERROR_CODE", got "${error.code}"`,
      );
    }
    ...
    try {
      ...
      if (!(error instanceof AdblockGeneratorError)) {
        const symptom = new Error(
          `Expected AdblockGeneratorError, got ${error}`,
      ...
      if (error.code !== "MISSING_SOURCE_FILE") {
        const symptom = new Error(
          `Expected error code MISSING_SOURCE_FILE, got ${error.code}`,
        ) as Error & { cause?: unknown };
        symptom.cause = error;
        throw symptom;
      }
  ...
  runner.test("File generation (integration test)", async () => {
    ...
    try {
      ...
      const domainResult = await generator.generateDomainList(
        rules,
        domainFile,
      );
